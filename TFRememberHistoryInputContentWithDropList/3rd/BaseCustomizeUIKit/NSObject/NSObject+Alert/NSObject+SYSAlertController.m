...
+(void)showSYSAlertViewTitle:(nullable NSString *)title
    ...
    for (int i = 0; i < alertBtnActionArr.count; i++) {
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:btnTitleArr[i]
            ...
            @strongify(targetVC)
            SuppressWarcPerformSelectorLeaksWarning([targetVC performSelector:NSSelectorFromString([NSString ensureNonnullString:alertBtnActionArr[i] ReplaceStr:@"defaultFunc"]) withObject:Nil]);
        }];
